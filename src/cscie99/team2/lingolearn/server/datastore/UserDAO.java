...
public class UserDAO {
	...
	private static final Language COURSE_LANG = new Language(LanguageTypes.English.toString());
	 ...
	 * 
	 * @param user
	 *            User object to be stored in datastore
	 ...
	 * 
	 * @param gmail
	 *            gmail address associated with the user
	 * @return user if datastore has it, null otherwise
	 */
	public User getUserByGmail(String gmail) {
		ObjectifyableUser oUser = ofy().load().type(ObjectifyableUser.class)
				.filter("gmail", gmail).first().now();
		return (oUser != null) ? oUser.getUser() : null;
	}

	/**
	 * Obtains a list of all available Users with specified native language in
	 * the datastore
	 * 
	 * @param lang
	 *            native language of the User
	 * @return List of all Users in the datastore having the same language or
	 *         null if nothing found
	 */
	public List<User> getAllUsersByNativeLanguage(String lang) {
		List<ObjectifyableUser> oUsers = ofy().load()
				.type(ObjectifyableUser.class)
				.filter("nativeLanguage.langName", lang).list();
	...
	/**
	 * Obtains a list of all available Users of specified gender in the
	 * datastore
	 * 
	 * @param gender
	 * @return List of all Users in the datastore having of the same gender or
	 *         null if nothing found
	 */
	public List<User> getAllUsersByGender(Gender gender) {
		List<ObjectifyableUser> oUsers = ofy().load()
				.type(ObjectifyableUser.class).filter("gender", gender).list();
		Iterator<ObjectifyableUser> it = oUsers.iterator();
		List<User> users = new ArrayList<>();
		while (it.hasNext()) {
			users.add(it.next().getUser());
	...
	/**
	 * Obtains a list of all available Users which previously studied specified
	 * language
	 * 
	 * @param lang
	...
	public List<User> getAllUsersByLanguage(String lang) {
		List<ObjectifyableUser> oUsers = ofy().load()
				.type(ObjectifyableUser.class)
				.filter("languages.langName =", lang).list();
		Iterator<ObjectifyableUser> it = oUsers.iterator();
		List<User> users = new ArrayList<>();
		while (it.hasNext()) {
			users.add(it.next().getUser());
	...
	/**
	 * Obtains a list of all available Users which previously used specified
	 * book
	 * 
	 * @param book
	 *            previously used book
	 * @return List of all Users in the datastore or null if nothing found
	 */
	public List<User> getAllUsersByTextbook(String book) {
		List<ObjectifyableUser> oUsers = ofy().load()
				.type(ObjectifyableUser.class).filter("textbooks.name =", book)
				.list();
		Iterator<ObjectifyableUser> it = oUsers.iterator();
		List<User> users = new ArrayList<>();
		while (it.hasNext()) {
	...
	/**
	 * Obtains a list of all available Users which previously took specified
	 * course
	 * 
	 * @param course
	 *            previously used course
	 * @return List of all Users in the datastore or null if nothing found
	 */
	public List<User> getAllUsersByOutsideCourse(String course) {
		List<ObjectifyableUser> oUsers = ofy().load()
				.type(ObjectifyableUser.class)
				.filter("outsideCourses.name =", course).list();
		Iterator<ObjectifyableUser> it = oUsers.iterator();
		List<User> users = new ArrayList<>();
		while (it.hasNext()) {
			users.add(it.next().getUser());
	...
	/**
	 * Obtains a list of all available Users which previously took a course at a
	 * specified institution
	 * 
	 * @param course
	 *            previously taken at this institution
	 * @return List of all Users in the datastore or null if nothing found
	 */
	public List<User> getAllUsersByOutsideInstitution(String inst) {
		List<ObjectifyableUser> oUsers = ofy().load()
				.type(ObjectifyableUser.class)
				.filter("outsideCourses.institution =", inst).list();
		Iterator<ObjectifyableUser> it = oUsers.iterator();
		List<User> users = new ArrayList<>();
		while (it.hasNext()) {
			users.add(it.next().getUser());
	...
	public List<User> getAllUsers() {
		List<ObjectifyableUser> oUsers = ofy().load()
				.type(ObjectifyableUser.class).list();
		Iterator<ObjectifyableUser> it = oUsers.iterator();
		List<User> users = new ArrayList<>();
		while (it.hasNext()) {
			users.add(it.next().getUser());
