...
public class SessionPresenter implements Presenter {
	...
	public SessionPresenter(CourseServiceAsync courseService,
			CardServiceAsync cardService, User currentUser,
			HandlerManager eventBus, SessionView display) {
		this.courseService = courseService;
		this.cardPresenter = new CardPresenter(cardService, eventBus,
				new CardView());
		this.quizPresenter = new QuizPresenter(cardService, eventBus,
				new QuizView(), this);
			...
			public void onPreviewNativeEvent(NativePreviewEvent event) {
				...
				if (event.getTypeInt() == Event.ONKEYDOWN) {
					if (ne.getKeyCode() == KeyCodes.KEY_ENTER
							|| ne.getKeyCode() == KeyCodes.KEY_SPACE) {
						ne.preventDefault();
					...
					if (session instanceof Lesson) {
						if (ne.getKeyCode() == KeyCodes.KEY_ONE
								|| ne.getKeyCode() == KeyCodes.KEY_NUM_ONE) {
							ne.preventDefault();
							recordKnowledge(Assessment.NOCLUE);
						} else if (ne.getKeyCode() == KeyCodes.KEY_TWO
								|| ne.getKeyCode() == KeyCodes.KEY_NUM_TWO) {
							ne.preventDefault();
							recordKnowledge(Assessment.SORTAKNEWIT);
						} else if (ne.getKeyCode() == KeyCodes.KEY_THREE
								|| ne.getKeyCode() == KeyCodes.KEY_NUM_THREE) {
							ne.preventDefault();
							recordKnowledge(Assessment.DEFINITELYKNEWIT);
						}
					} else {
						if (ne.getKeyCode() == KeyCodes.KEY_ONE
							...
							ne.preventDefault();
							quizPresenter.getDisplay().setSelectedAnswer(
									ne.getKeyCode() - KeyCodes.KEY_ONE);
						}
	...
	public void go(final HasWidgets container) {
		...
		try {
			...
			Notice.showNotice("The study session specified is invalid.",
					"error");
		}
	}
				...
				courseService.getCourseById(returnedSession.getCourseId(),
						new AsyncCallback<Course>() {
							...
							public void onFailure(Throwable caught) {
								Notice.showNotice(
										"Unable to get course of session.",
										"warning");
							}
							...
							public void onSuccess(Course result) {
								...
								// query string, or from the quiz instance
								// variable
								// set by the instructor
								String sessionType = Window.Location
								...
								if (session instanceof Quiz) {
									...
									try {
										...
										Notice.showNotice(
												"Unable to parse the session type ("
														+ type + ")", "error");
										return;
									}
								}
								...
								courseService.createUserSession(
										...
										new AsyncCallback<UserSession>() {
											...
											public void onSuccess(
												...
												if (session instanceof Lesson) {
													cardPresenter.go(display
															.getCardContainer());
												} else {
													quizPresenter.go(display
															.getCardContainer());
												}

												display.setSessionName(session
														.getDeck().getDesc());
											...
											public void onFailure(
													Throwable caught) {
												Window.alert("An unhandled error occured: "
														+ caught.getMessage());
											}
	...
	private ArrayList<Long> selectThreeOtherCardsFromDeck() {
		...
		while ((selectedIds.size() < 3)
				&& (selectedIds.size() != (allIds.size() - 1))) {
