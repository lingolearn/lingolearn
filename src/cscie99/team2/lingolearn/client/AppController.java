...
public class AppController implements Presenter, ValueChangeHandler<String> {
	...
	public AppController(UserServiceAsync userService,
			CourseServiceAsync courseService, CardServiceAsync cardService,
	...
	private void bind() {
		...
		eventBus.addHandler(ViewCardEvent.TYPE, new ViewCardEventHandler() {
			public void onViewCard(ViewCardEvent event) {
				doViewCard();
			}
		});

		eventBus.addHandler(AnalyticsEvent.TYPE, new AnalyticsEventHandler() {
							...
							analyticsService.storeQuizResponse(quizResponse,
									new AsyncCallback<QuizResponse>() {
										...
										public void onFailure(Throwable caught) {
											System.out
													.println("failed to store quiz response");
										}
				...
				userService.getCurrentUser(new AsyncCallback<User>() {
					public void onSuccess(User user) {
						if (user instanceof User) {
							...
							analyticsService.storeFlashCardResponse(
									flashCardResponse,
									new AsyncCallback<FlashCardResponse>() {

										@Override
										public void onFailure(Throwable caught) {
											System.out
													.println("failed to store flash card response");
										}
										...
										public void onSuccess(
											...
											System.out
													.println("successfully stored flash response");
										}
	...
	private void loadPresentationView(ValueChangeEvent<String> event) {
		...
		if (token != null) {
			...
			case "viewCard":
				presenter = new CardPresenter(cardService, eventBus,
						new CardView());
				break;
			case "home":
			...
			case "enrollInCourse":
				presenter = new EnrollInCoursePresenter(courseService,
						currentUser, eventBus, new EnrollInCourseView());
				break;
			case "session":
				presenter = new SessionPresenter(courseService, cardService,
						currentUser, eventBus, new SessionView());
				break;
			case "register":
	 ...
	 *            Indicates if the user is logged in or not. If the user is not
	...
	/*
	 * Check to see that a user is logged into this session. This is performed
	 * by calling the user service. One the async method returns, we can bring
	 * the user to a page (to use the app, or login).
	 * 
	...
	/**
	 * Retrieve the Google ID information (gmail, and gplusid). If this
	 * information is retrieved, then the app will attempt to login the user.
	 */
	...
	/**
	 * Attempt to login the session user with the provided Google ID information
	 * (gmail, gplus). On success, the user will be logged in, if the user with
	 * the google id info doesn't exist, they will be sent to the registration
	 * page.
	 * 
	 * @param gpack
	 */
